...
namespace BurnIn
{
    public partial class frm_server : Form
    {
        ...
        // 选择要发送的文件
        private void btnSelectFile_Click_1(object sender, EventArgs e)//Select send file
        {
        ...
        // 文件的发送
        private void btnSendFile_Click_1(object sender, EventArgs e)//send file
        {
            ...
            else
            {
                ...
                using (FileStream fs = new FileStream(txtSelectFile.Text, FileMode.Open, FileAccess.Read))
                {
                    ...
                    string strMsg = "Send file name is： " + fileName + "\r\n";
                    byte[] arrMsg = System.Text.Encoding.UTF8.GetBytes(strMsg); // 将要发送的字符串转换成Utf-8字节数组；
                    byte[] arrSendMsg = new byte[arrMsg.Length + 1];
                    arrSendMsg[0] = 0; // 表示发送的是消息数据
                    Buffer.BlockCopy(arrMsg, 0, arrSendMsg, 1, arrMsg.Length);
